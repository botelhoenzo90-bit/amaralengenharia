...
const waterproofingImage = asset("service-waterproofing.jpg");
const engineeringImage = asset("service-engineering.jpg");
const renovationImage = asset("service-renovation.jpg");
const guttersImage = asset("service-gutters.jpg");
const leisureImage = asset("service-leisure.jpg");
...
const reviews = [
  ["Carlos M.", "Fiquei muito satisfeito com o atendimento. A avaliação foi detalhada, os pontos do imóvel foram explicados com clareza e soube exatamente quais eram os próximos passos."],
  ["Mariana R.", "O atendimento foi muito profissional do início ao fim. Recebi orientações claras e uma análise cuidadosa para resolver o problema do imóvel com mais segurança."],
  ["Roberto A.", "Precisava de uma avaliação para entender um problema no imóvel e fui muito bem atendido. A comunicação foi objetiva e transmitiu bastante confiança."],
  ["Fernanda S.", "Meu telhado apresentava problemas e eu não sabia por onde começar. A orientação foi clara, profissional e ajudou bastante na tomada de decisão."],
  ["Paulo C.", "Gostei muito da forma como o serviço foi explicado. Atendimento direto, organizado e com atenção aos detalhes que realmente importam no imóvel."],
  ["Juliana F.", "Foi ótimo encontrar diferentes soluções de engenharia no mesmo contato. O atendimento passou segurança e mostrou preocupação em encontrar a solução adequada."],
] as const;
...
function ServiceCard({ item }: { item: typeof services[number] }) {
  ...
  return <article className="ae-service-card"><div className="ae-service-image"><img src={image} alt={String(title)} loading="lazy"/></div><span className="ae-service-icon"><ServiceIcon/></span><div className="ae-service-body"><h3>{String(title)}</h3><p>{String(text)}</p></div></article>;
}
...
function Index() {
  return <div className="ae-site">
    <main>
      ...
      <section className="ae-section ae-reviews" id="avaliacoes"><div className="ae-container"><div className="ae-section-heading"><div><span className="ae-kicker">AVALIAÇÕES</span><h2>Confiança construída no atendimento.</h2></div><p>Experiências de clientes atendidos pela Amaral Engenharia.</p></div><div className="ae-marquee-wrap"><div className="ae-marquee ae-reviews-marquee">{marqueeReviews.map((item,i)=><ReviewCard item={item} key={`${item[0]}-${i}`}/>)}</div></div><div className="ae-centered-cta"><ContactButton label="Falar com a Amaral Engenharia" message="Olá! Vim pelas avaliações do site e quero conhecer os serviços."/></div></div></section>

      <section className="ae-section ae-process"><div className="ae-container"><div className="ae-section-heading"><div><span className="ae-kicker">COMO FUNCIONA</span><h2>Um atendimento simples para começar.</h2></div><p>Explique sua necessidade e receba orientação sobre o próximo passo.</p></div><div className="ae-process-highlight"><strong>Do primeiro contato à execução</strong><span>Você sabe o que acontece em cada etapa, sem complicação e sem excesso de burocracia.</span></div><div className="ae-process-grid">{[["01","Contato","Envie uma mensagem e explique o que precisa."],["02","Entendimento","A necessidade do imóvel é compreendida."],["03","Avaliação","O serviço adequado é definido conforme o caso."],["04","Execução","A solução é planejada e executada."]].map(([n,t,d])=><article key={n}><span>{n}</span><div className="ae-process-icon"><Check/></div><h3>{t}</h3><p>{d}</p></article>)}</div><div className="ae-centered-cta"><ContactButton label="Começar atendimento pelo WhatsApp" message="Olá! Quero começar um atendimento com a Amaral Engenharia."/></div></div></section>
    ...
    <a className="ae-floating-whatsapp" href={wa("Olá! Vim pelo site da Amaral Engenharia e quero solicitar atendimento.")} target="_blank" rel="noreferrer" aria-label="Falar com a Amaral Engenharia pelo WhatsApp"><img className="ae-whatsapp-logo" src={whatsappLogo.url} alt="WhatsApp"/></a>
